...
import fr.colline.monatis.controller.dto.TitulaireRequestDto;
import fr.colline.monatis.controller.dto.TitulaireResponseDto;
import fr.colline.monatis.controller.dto.mapper.TitulaireDtoMapper;
...
public class TitulaireController {
	...
	@GetMapping("/all")
	public List<TitulaireResponseDto> getAllTitulaires() 
			throws ControllerException {

		try {
			List<TitulaireResponseDto> resultat = new ArrayList<>();
			Sort tri = Sort.by("nom");
			List<Titulaire> liste = titulaireService.rechercherTous(tri);
			for ( Titulaire titulaire : liste ) {
				resultat.add(TitulaireDtoMapper.detailedModelToResponseDto(titulaire));
			}
			return resultat;
		} 
	...
	@GetMapping("/get/{nom}")
	public TitulaireResponseDto getTitulaireParNom(@PathVariable(name = "nom") String nom) 
			throws ControllerException {
		...
		try {
			...
			return TitulaireDtoMapper.detailedModelToResponseDto(titulaire);
		} 
		catch (ServiceTechniqueException e) {
			throw new ControllerException(
					e,
	...
	@PostMapping("/new")
	public TitulaireResponseDto creerTitulaire(@RequestBody TitulaireRequestDto dto) 
			throws ControllerException {
	...
	@PutMapping("/mod/{nom}")
	public TitulaireResponseDto modifierTitulaire(
